...
def build_statement(account: dict, pending: list) -> dict:
    """Picks the earliest pending cheque to show as cleared, works out a
    closing balance consistent with that, and returns everything the PDF
    (and the parser, afterwards) needs."""
    today = date.today()
...
def render_pdf(account: dict, statement: dict, out_path: Path):
    ...
    if statement["cleared_cheque"]:
        ...
        rows.append(["-", "No cheques cleared this period", "-"])
...
def main():
    ...
    statement = build_statement(account, pending)
