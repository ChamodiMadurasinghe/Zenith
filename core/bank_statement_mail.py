...
def _content_looks_like_statement(text: str) -> bool:
    """Fallback for when the subject doesn't say 'statement' but the actual
    attachment/body text does — real banks are inconsistent about subject
    wording, so this is checked in addition to (not instead of) the subject."""
    text_l = (text or "").lower()
    return any(k in text_l for k in STATEMENT_KEYWORDS)
...
class MailConnectionError(Exception):
    """Raised when the IMAP connection/login itself fails — wrong password,
    blocked login, wrong host/port, network issue, etc. Kept separate from
    'no statement found' (that's a normal, successful, empty result)."""
...
def fetch_latest_statement_email(config: dict) -> dict | None:
    ...
    Raises MailConnectionError if the connection/login itself fails, so the
    caller can show "couldn't log in" instead of "no statement found".
    """
    try:
        conn = imaplib.IMAP4_SSL(config["imap_host"], int(config["imap_port"]))
    except (OSError, imaplib.IMAP4.error) as e:
        raise MailConnectionError(f"Could not connect to {config['imap_host']}:{config['imap_port']} ({e})") from e

    try:
        conn.login(config["business_email"], config["business_email_app_password"])
    except imaplib.IMAP4.error as e:
        raise MailConnectionError(
            "Login failed — check the business email and app password (a normal Gmail "
            f"password will not work over IMAP). Server said: {e}"
        ) from e
    ...
    try:
        ...
        for msg_id in reversed(ids[-30:]):  # newest first, last 30 from that sender
            ...
            # A transaction alert is never treated as the monthly statement,
            # regardless of what's in the attachment — subject rules this out.
            if any(k in subject_l for k in TRANSACTION_KEYWORDS):
                continue

            # Real banks don't all phrase the subject the same way, so don't
            # rely on the subject alone: if it doesn't obviously say
            # "statement", check the attachment/body content too before
            # giving up on this email.
            text = _extract_statement_text(msg)
            subject_says_statement = any(k in subject_l for k in STATEMENT_KEYWORDS)
            content_says_statement = _content_looks_like_statement(text)
            if not (subject_says_statement or content_says_statement):
                continue
...
def run_verification(account_id: int, is_test: bool = False) -> dict:
    ...
    if is_test:
        ...
        try:
            ...
            return {"error": "mail_connection_failed", "detail": str(e)}
        except Exception as e:  # noqa: BLE001 — never let a mailbox hiccup 500 the page
