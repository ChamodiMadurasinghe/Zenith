...
class TestContentFallback(unittest.TestCase):
    def test_recognises_statement_from_attachment_text_even_with_unrelated_subject(self):
        # This is the exact bug: subject has no "statement" wording, but the
        # attached PDF's own text clearly is one.
        self.assertTrue(_content_looks_like_statement(sample_statement_text()))

    def test_does_not_recognise_unrelated_text(self):
        self.assertFalse(_content_looks_like_statement("Thanks for your order! Here's your receipt."))

    def test_empty_text_is_not_a_statement(self):
        self.assertFalse(_content_looks_like_statement(""))
        self.assertFalse(_content_looks_like_statement(None))
